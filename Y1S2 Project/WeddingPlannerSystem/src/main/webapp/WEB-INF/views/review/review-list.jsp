...
<head>
    ...
    <title>Review Management — Wedding Planner</title>
...
<div class="container my-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="bi bi-stars me-2"></i>All Reviews</h2>
        <a href="${pageContext.request.contextPath}/reviews?action=add" class="btn btn-primary">
    ...
    <c:choose>
        <c:when test="${empty reviews}">
            <div class="alert alert-info">
                <i class="bi bi-info-circle me-1"></i>No reviews found. Be the first to review!
            </div>
        </c:when>
        <c:otherwise>
                        ...
                        <div class="card h-100 shadow-sm">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <h6 class="card-subtitle text-muted">Vendor ID: ${review.vendorId}</h6>
                                    <span class="text-warning fw-bold">
                                        <c:forEach begin="1" end="${review.rating}"><i class="bi bi-star-fill"></i></c:forEach>
                                        <c:forEach begin="${review.rating + 1}" end="5"><i class="bi bi-star text-muted"></i></c:forEach>
                                    </span>
                                </div>
                                <p class="card-text">"${review.comment}"</p>
                                <p class="card-text mt-2"><small class="text-muted">Reviewed by User ID: ${review.userId}</small></p>
                            </div>
                            <div class="card-footer bg-white border-top-0 d-flex justify-content-end">
                                <a href="${pageContext.request.contextPath}/reviews?action=edit&id=${review.id}" class="btn btn-sm btn-outline-warning me-2" title="Edit">
                                    <i class="bi bi-pencil"></i> Edit
                                </a>
                                <a href="${pageContext.request.contextPath}/reviews?action=delete&id=${review.id}" class="btn btn-sm btn-outline-danger" title="Delete" onclick="return confirm('Are you sure you want to delete this review?');">
